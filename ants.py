...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    self.ant = insect #place the insect that now holds the origianl ant into the place
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.is_realqueen:
                return None
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        #  Special handling for bee direction
        """
        if self.direction == -1:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while placemarker < self.min_range:
            placemarker += 1
            curr_place = curr_place.entrance

        while not curr_place.bees and curr_place.entrance != hive:
            curr_place = curr_place.entrance
            placemarker += 1

        if self.min_range <= placemarker <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        list_of_bees = self.place.bees #list of all the bees in the current place
        Ant.reduce_armor(self, amount)
        if self.armor <= 0:
            for b in list(list_of_bees):
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt acts defensively with think armor"""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1, is_realqueen = False):
        type(self).num_queen += 1
        self.armor = armor
        self.is_realqueen = is_realqueen
        if self.num_queen == 1:
    ...
    def action(self, colony):
        ...
        p = self.place.exit
        while p:
            places_before_queen.append(p)
            p = p.exit

        if self.is_realqueen == False:
            self.armor = 0
            Insect.reduce_armor(self, 0)
        else:
            ThrowerAnt.action(self, colony)
            for p in places_before_queen:
                if p.ant:
                    #First double damage of the innermost ant (contained ant if there is one)
                    if p.ant.is_container and p.ant.contained_ant and p.ant.contained_ant.is_buffed == False:
                        p.ant.contained_ant.damage = p.ant.contained_ant.damage * 2
                        p.ant.contained_ant.is_buffed = True
                    #Then double the ant at the place, whether that's the ant to begin with or the container ant
                    if p.ant.is_buffed == False:
                        p.ant.damage = p.ant.damage * 2
                        p.ant.is_buffed = True
...
def make_scare(action, bee):
    ...
    def scare(colony):
        ...
        if bee.is_scared == False:
            times += 1
            if times >= 2: # bee can be scared for two turns
                bee.is_scared = True
...
def apply_effect(effect, bee, duration):
    ...
    effected_action = effect(original_action, bee)
    i = 0
